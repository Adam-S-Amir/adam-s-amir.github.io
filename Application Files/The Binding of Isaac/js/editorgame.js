	...
	this.draw = function (context) {
		...
		if (!this.isDestroyed) {
			...
			if (this.type == "xblock") context.drawImage(imageTool.xblock, this.x - 3, this.y + 3, 64, 74);
				...
				switch (this.imgRand) {
					case 1:
						context.drawImage(imageTool.block, this.x - 3, this.y + 3, 64, 74);
						break;
					case 2:
						context.drawImage(imageTool.block1, this.x - 3, this.y + 3, 64, 74);
						break;
					case 3:
						context.drawImage(imageTool.block2, this.x - 3, this.y + 3, 64, 74);
						break;
					default:
						context.drawImage(imageTool.block, this.x - 3, this.y + 3, 64, 74);
						break;
...
function Poop(x, y) {
	...
	this.x = x + 4;
	this.y = y + 4;
	this.state = 0; //�tapes de destruction
	this.width = 50;
	this.height = 50;
	this.canBeDestroyed = true;
	this.isDestroyed = false;
	...
	this.draw = function (context) {
		...
		if (this.state < 1) context.drawImage(imageTool.poop1, this.x - 12, this.y - 10, 78, 78);
			...
			context.drawImage(imageTool.poop5, this.x - 12, this.y - 3, 78, 78);
			if (this.canAnim) {
	...
	this.create = function () {
		if (!this.isCreated) {
			...
			if (rand < 2 && Game.currentShopPool.length > 0) {
				...
				console.log('Tentative de cr�ation dun item');
...
function Chest(x, y, type) {
	...
	this.PlayerSliding = false;
	this.ExploSliding = false;
	this.canBeUsed = true; //Ferm� ou ouvert
	this.alive = true;
	this.isColliding = true;
	this.update = function () {
		this.checkCollide(Player);
...
function Room(type, map, locy, locx) {
	this.type = type;
	this.locy = locy; //Position Y dans l'array d'�tage
	this.locx = locx; // Position X dans l'array d'�tage
	this.grid = [];
	...
	this.enemies = 0; //Total d'ennemies pr�sents
	this.combatMode = 1; //Mode de combat
	this.lootx = 0; //Positionx du loot de room
	this.looty = 0; //Positiony du loot de room
	this.canSpawnLoot = false; // D�termine si la room peut donner du loot ou pas
	this.exists = true; //Propri�t� pour filtrer l'array des salles existantes et des emplacements vides
	this.isVisited = false; //Minimap, la salle a �t� visit�
	this.isCurrent = false; //Minimap, la salle actuelle
	this.isVisible = false; //Minimap, la salle est visible (adjacente � une salle visit�e, ou le joueur poss�de une carte)
	this.iconVisible = false; //Minimap, l'icone de la salle est visible (adjacente � une salle visit�e, ou le joueur poss�de une boussole)
	...
	this.create = function () {
		for (var i = 0, y = 0; i < this.map.length; i++, y += 64) {
			...
			for (var j = 0, x = 0; j < this.map[i].length; j++, x += 64) {
				...
				this.grid[i][j] = 0;
				if (this.map[i][j] == "Pl") {
					this.freeCells.push(new freeCell(x, y));
					Player.x = x;
					Player.y = y;
				...
				//Obstacles
					...
					this.traps.push(new Glue(x, y));
					this.grid[i][j] = 1;
				} else if (this.map[i][j] == "Ts") {
					this.traps.push(new Spikes(x, y));
					this.grid[i][j] = 1;
				} else if (this.map[i][j] == "!!") {
					...
					this.collideMaps.push(new Block(x, y));
					this.grid[i][j] = 0;
				} else if (this.map[i][j] == "Oo") {
					this.collideMaps.push(new Poop(x, y));
					this.grid[i][j] = 0;
				} else if (this.map[i][j] == "Tn") {
					this.collideMaps.push(new Tnt(x, y));
					this.grid[i][j] = 0;
				} else if (this.map[i][j] == "Tf") {
					this.collideMaps.push(new Fireplace(x, y, "fireplace"));
					this.grid[i][j] = 0;
				} else if (this.map[i][j] == "Th") {
					this.collideMaps.push(new Fireplace(x, y, "hellfireplace"));
					this.grid[i][j] = 0;
					...
					this.holeMaps.push(new Hole(x, y, this.map[i][j]));
					this.grid[i][j] = 0;
				}
				//Ennemis
				else if (this.map[i][j] == "Sp") {
					this.Minions.push(new Spider(x + 15, y + 25, 1.5, "spider", true));
					this.grid[i][j] = 1;
				} else if (this.map[i][j] == "Sb") {
					this.Minions.push(new Spider(x + 15, y + 25, 3.25, "buttspider", true));
					this.grid[i][j] = 1;
				} else if (this.map[i][j] == "Zo") {
					this.Minions.push(new Zombie(x + 10, y + 20, 3.25));
					this.grid[i][j] = 1;
				} else if (this.map[i][j] == "Cl") {
					this.Minions.push(new Clotty(i, j, x + 10, y + 20, 5.5));
					this.grid[i][j] = 1;
				} else if (this.map[i][j] == "Ma") {
					this.Minions.push(new Maggot(i, j, x + 10, y + 20, 3.25));
					this.grid[i][j] = 1;
				} else if (this.map[i][j] == "Fl") {
					this.Minions.push(new Fly(x, y, 1.25, "Black", true));
					this.grid[i][j] = 1;
				} else if (this.map[i][j] == "Af") {
					this.Minions.push(new Fly(x, y, 1.25, "Attack", true));
					this.grid[i][j] = 1;
				} else if (this.map[i][j] == "Pf") {
					this.Minions.push(new Fly(x, y, 2.5, "Pooter", true));
					this.grid[i][j] = 1;
				} else if (this.map[i][j] == "To") {
					this.Towers.push(new Tower(x, y, 4.25, true));
					this.grid[i][j] = 1;
				}
				//Bosses
				else if (this.map[i][j] == "X1") {
					this.Bosses.push(new Duke(x, y, 70, 1));
					this.grid[i][j] = 1;
				} else if (this.map[i][j] == "X3") {
					this.Bosses.push(new Duke(x, y, 120, 2));
					this.grid[i][j] = 1;
				} else if (this.map[i][j] == "X2") {
					this.Bosses.push(new Project(422, 230, 180));
					this.grid[i][j] = 1;
				...
				//Items
					...
					this.Items.push(new Item(x + 16, y + 16, "Health"));
					this.grid[i][j] = 1;
				}
			}
		}
	...
	this.update = function () {
		...
		for (var xb = 0; xb < this.Bosses.length; xb++) {
			...
			if (!this.Bosses[xb].alive) {
				this.Bosses.splice(xb, 1);
				gameStats.kill++;
			}
		}
	...
	this.draw = function () {
		...
		if (updatingBackground) {
			...
			for (var h = 0; h < this.holeMaps.length; h++) {
				this.holeMaps[h].draw(bgcontext);
			}
		}
		for (var c = 0; c < this.collideMaps.length; c++) {
		...
		if (!gameOver) {
			...
			for (var t = 0; t < this.Towers.length; t++) {
				this.Towers[t].draw(context);
			}
			for (var b = 0; b < playerBulletsBack.length; b++) {
				playerBulletsBack[b].draw(context);
			}
			for (var u = 0; u < this.Bombs.length; u++) {
				this.Bombs[u].draw(context);
			}
		}
		...
		if (!gameOver) {
			for (var xb = 0; xb < this.Bosses.length; xb++) {
				this.Bosses[xb].draw(context);
			}
			for (var u = 0; u < this.Explosions.length; u++) {
				this.Explosions[u].draw(context);
			}
			for (var tb = 0; tb < this.enemyBullets.length; tb++) {
	...
	this.reset = function () {
		for (var i = 0; i < this.Minions.length; i++) {
			this.Minions[i].reset();
		}
		for (var t = 0; t < this.Towers.length; t++) {
			this.Towers[t].reset();
		}
		for (var xb = 0; xb < this.Bosses.length; xb++) {
			this.Bosses[xb].reset();
		}
		this.enemyBullets = [];
		this.Bombs = [];
		playerBullets = [];
