...
var Player = {
	...
	orientation: "down", //Orientation de la t�te
	blink: 0, //Clignotement quand damag�
	isShooting: false, //Animation de tir
	isLooting: false, //Animation de loot
	textShowing: false, //Description d'item
	itemHolding: "", //L'item qui est loot�
	itemHoldingName: "", //Le nom de l'item qui est loot�
	itemHoldingDesc: "", //La description de l'item qui est loot�
	...
	update: function () {
		...
		if (!this.alive) gameOver = true; //D�tection de fin de jeu

		//Gestion de minimap
		if (this.TreasureMap) showMap("Treasure Map"); //Montre les pi�ces
		if (this.TheCompass) showMap("The Compass"); //Montre les icones de pi�ces sp�ciales
		Game.isVisited = true;
		Game.isVisible = true;
		...
		//State pour l'invuln�rabilit� temporaire
		if (Date.now() - this.lastDamaged > 700) {
			this.canGetDamage = true;
		} //compare le temps actuel avec le temps du dernier d�gat
		...
		//D�c�l�ration, si les deux touches d'une meme dimension sont relach�es ou enfonc�es(elles s'annulent)
		if (this.isLooting) {
			if (this.accely != 0) {
				this.accely -= this.accely / 7;
			}
			if (this.accelx != 0) {
				this.accelx -= this.accelx / 7;
			}
		}
	...
	drawHead: function (context) {
		if (!this.isLooting) {
			//Joueur invuln�rable (a recu du damage)
			...
			//Remettre la t�te de face apr�s un timer
	...
	drawUI: function (context, uicontext) {
		//Interface
		if (Date.now() - this.uiUpdated > 200) { // Draw � toutes les x millisecondes
			...
			for (var h = 0; h < intHp; h++) {
				...
				uicontext.drawImage(imageTool.fullhp, (pool * width) + ox - col, oy + row, width, height);
				pool++;
			}
			if (half != 0) {
				if (pool > 29) {
				...
				uicontext.drawImage(imageTool.halfHp, (pool * width) + ox - col, oy + row, width, height);
				pool++;
			}
			for (var d = 0; d < diffHp; d++) {
				...
				uicontext.drawImage(imageTool.emptyHp, (pool * width) + ox - col, oy + row, width, height);
				pool++;
			}
			for (var a = 0; a < intArmor; a++) {
				...
				uicontext.drawImage(imageTool.ap, (pool * width) + ox - col, oy + row, width, height);
				pool++;
			}
			if (halfarmor != 0) {
				if (pool > 29) {
				...
				uicontext.drawImage(imageTool.halfAp, (pool * width) + ox - col, oy + row, width, height);
				pool++;
			}
			// Gold, Keys, Bombs
			...
			if (!editorMode) {
				...
				for (var y = 0; y < currentFloor.length; y++) {
					for (var x = 0; x < currentFloor[y].length; x++) {
						if (currentFloor[y][x].isCurrent && currentFloor[y][x].type != "Secret") {
							uicontext.drawImage(imageTool.current, (x * 42) + left, (y * 17) + top, 44, 21);
						} else if (currentFloor[y][x].exists && currentFloor[y][x].isVisited && currentFloor[y][x].type != "Secret") {
							uicontext.drawImage(imageTool.visited, (x * 42) + left, (y * 17) + top, 44, 21);
						} else if (currentFloor[y][x].exists && currentFloor[y][x].isVisible && currentFloor[y][x].type != "Secret") {
							uicontext.drawImage(imageTool.unvisited, (x * 42) + left, (y * 17) + top, 44, 21);
						}
					}
				}
				for (var y = 0; y < currentFloor.length; y++) {
					for (var x = 0; x < currentFloor[y].length; x++) {
						if (currentFloor[y][x].exists && currentFloor[y][x].iconVisible && currentFloor[y][x].type == "Boss") {
							uicontext.drawImage(imageTool.boss, (x * 42) + left, (y * 17) + top - 8, 44, 32);
						} else if (currentFloor[y][x].exists && currentFloor[y][x].iconVisible && currentFloor[y][x].type == "Treasure") {
							uicontext.drawImage(imageTool.treasure, (x * 42) + left, (y * 17) + top - 8, 44, 32);
						} else if (currentFloor[y][x].exists && currentFloor[y][x].iconVisible && currentFloor[y][x].type == "Shop") {
							uicontext.drawImage(imageTool.shop, (x * 42) + left + 10, (y * 17) + top - 4, 26, 28);
						} else if (currentFloor[y][x].exists && currentFloor[y][x].iconVisible && currentFloor[y][x].type == "Sacrifice") {
							uicontext.drawImage(imageTool.sacrifice, (x * 42) + left + 10, (y * 17) + top - 4, 26, 28);
						} else if (currentFloor[y][x].exists && currentFloor[y][x].iconVisible && currentFloor[y][x].type == "Cursed") {
							uicontext.drawImage(imageTool.cursed, (x * 42) + left + 10, (y * 17) + top - 4, 26, 28);
						} else if (currentFloor[y][x].exists && (currentFloor[y][x].isVisible || currentFloor[y][x].isVisible) && currentFloor[y][x].type == "Secret") {
							uicontext.drawImage(imageTool.secret, (x * 42) + left, (y * 17) + top - 4, 44, 26);
						}

						if (currentFloor[y][x].exists && currentFloor[y][x].chestPresent) {
							uicontext.drawImage(imageTool.minichest, (x * 42) + left + 16, (y * 17) + top + 3, 14, 14);
						}
						if (currentFloor[y][x].exists && currentFloor[y][x].hpPresent) {
							uicontext.drawImage(imageTool.minihp, (x * 42) + left + 16, (y * 17) + top + 3, 14, 14);
						}
						if (currentFloor[y][x].exists && currentFloor[y][x].coinPresent) {
							uicontext.drawImage(imageTool.minicoin, (x * 42) + left + 16, (y * 17) + top + 3, 14, 14);
						}
						if (currentFloor[y][x].exists && currentFloor[y][x].keyPresent) {
							uicontext.drawImage(imageTool.minikey, (x * 42) + left + 5, (y * 17) + top + 3, 14, 14);
						}
						if (currentFloor[y][x].exists && currentFloor[y][x].bombPresent) {
							uicontext.drawImage(imageTool.minibomb, (x * 42) + left + 26, (y * 17) + top + 3, 14, 14);
						}
					}
				}
			}
	...
	getDamage: function (dmg, enemyx, enemyy) {
		...
		if (this.alive) { //Si vivant
			...
			if (dmg > 0) {
				...
				this.damagedNow = Date.now(); //Moment ou le d�gat est pris
				if (this.damagedNow - this.lastDamaged > 1000) { //Si le dernier d�gat date d'une seconde
					if (this.soul > 0) {
						this.soul -= 0.5;
	...
	playerFire: function () {
		...
		if (Date.now() - this.lastFire > this.fireRate) {
			...
			//Si le joueur tire dans la direction qu'il avance, la vitesse et la port�e des projectiles est d�cupl�e
			...
			//Sinon par d�fault
				...
				var brange = this.range;
				var bspeed = this.attackSpeed;
			}
			gameStats.bullet++;
			if (this.isNumberOne) {
				gapSwitch = 4;
